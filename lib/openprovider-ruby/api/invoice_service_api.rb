...
module OpenProvider
  class InvoiceServiceApi
    ...
    # @option opts [String] :start_creation_date The start creation date.
    # @option opts [String] :end_creation_date The end creation date.
    # @return [Array<(InvoiceListInvoicesResponse, Integer, Hash)>] InvoiceListInvoicesResponse data, response status code and response headers
    def list_invoices_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: InvoiceServiceApi.list_invoices ...'
      end
      # resource path
      local_var_path = '/v1beta/invoices'
      ...
      return_type = opts[:return_type] || 'InvoiceListInvoicesResponse' 
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: InvoiceServiceApi#list_invoices\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
