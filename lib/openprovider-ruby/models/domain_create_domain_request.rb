  ...
  class DomainCreateDomainRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenProvider::DomainCreateDomainRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenProvider::DomainCreateDomainRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'accept_eap_fee')
        self.accept_eap_fee = attributes[:'accept_eap_fee']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          accept_eap_fee == o.accept_eap_fee &&
    ...
    def hash
      [accept_eap_fee, accept_premium_fee, additional_data, admin_handle, application_mode, application_notice_id, application_smd, auth_code, autorenew, billing_handle, comments, dnssec_keys, domain, is_dnssec_enabled, is_private_whois_enabled, is_sectigo_dns_enabled, is_spamexperts_enabled, name_servers, ns_group, ns_template_id, ns_template_name, owner_handle, period, promo_code, reseller_handle, tech_handle, unit, use_domicile].hash
    end
