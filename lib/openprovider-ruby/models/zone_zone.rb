  ...
  class ZoneZone
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenProvider::ZoneZone` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenProvider::ZoneZone`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'dnskey')
        self.dnskey = attributes[:'dnskey']
      end
      ...
      if attributes.key?(:'is_shadow')
        self.is_shadow = attributes[:'is_shadow']
      end
      ...
      if attributes.key?(:'premium_dns')
        self.premium_dns = attributes[:'premium_dns']
      end
    ...
    def hash
      [active, creation_date, dnskey, history, id, ip, is_deleted, is_shadow, is_spamexperts_enabled, modification_date, name, premium_dns, provider, records, reseller_id, type].hash
    end
