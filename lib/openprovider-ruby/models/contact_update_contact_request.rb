  ...
  class ContactUpdateContactRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenProvider::ContactUpdateContactRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenProvider::ContactUpdateContactRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [additional_data, address, api_access_enabled, api_client_ip_list, auth_type, comments, company_name, email, id, inn, is_active, kpp, locale, name, password_change_declined_at, phone, rcp_client_ip_list, role, type, vat].hash
    end
