  ...
  class CustomerGetCustomerResponseData
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenProvider::CustomerGetCustomerResponseData` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenProvider::CustomerGetCustomerResponseData`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [additional_data, address, comments, comments_last_changed_at, company_name, deleted_at, email, email_verification_status, extension_additional_data, fax, handle, id, inn, is_deleted, locale, name, phone, reseller_id, tags, vat].hash
    end
